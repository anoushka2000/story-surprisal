...
# ------------------------------------------------------------
# 1.  Load an open embedding model
# ------------------------------------------------------------
MODEL_NAME = "sentence-transformers/all-MiniLM-L6-v2"  # 384-D sentence embeddings
...
# ------------------------------------------------------------
# 2.  Load & segment a story
#     Replace "story.txt" with any text file containing fiction.
# ------------------------------------------------------------
with open("data/story.txt", encoding="utf-8") as f:
    text = f.read()

sentences: List[str] = sent_tokenize(text)               # ~1–2 ms per sentence

# ------------------------------------------------------------
# 3.  Compute embeddings (batched for speed)
# ------------------------------------------------------------
batch_size = 8
embeds = []
...
# ------------------------------------------------------------
# 4.  Cosine similarity of each sentence vs. the previous one
# ------------------------------------------------------------
# Vectorized pairwise: sim(i) = cos(sent_i, sent_{i-1})
prev = embeddings[:-1]
curr = embeddings[1:]
sims = cosine_similarity(curr, prev).diagonal()  # length = N-1

# ------------------------------------------------------------
# 5.  Flag low-similarity points (semantic "jumps")
# ------------------------------------------------------------
...
for idx, sim in enumerate(sims, start=1):   # idx == sentence index
    if sim < threshold:
        print(f"[{idx:>4}] sim={sim:.3f} → {sentences[idx][:120]} …")
